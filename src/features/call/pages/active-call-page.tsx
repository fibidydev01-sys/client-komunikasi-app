// ================================================
// FILE: src/features/call/pages/active-call-page.tsx
// ActiveCallPage - WITH WebRTC Audio/Video
// ================================================
...
export const ActiveCallPage = () => {
  ...
  useEffect(() => {
    if (!activeCall) {
      logger.warn('Active Call Page: No active call, redirecting...');
  ...
  // ============================================
  useEffect(() => {
    if (activeCall && !isInitialized) {
      logger.debug('Active Call Page: Initializing WebRTC...');
      initializeCall();
    }
  }, [activeCall, isInitialized, initializeCall]);
  ...
  useEffect(() => {
    if (localStream && localVideoRef.current) {
      ...
      logger.debug('Active Call Page: Local stream attached to video');
    }
  }, [localStream]);
  ...
  useEffect(() => {
    if (remoteStream && remoteVideoRef.current) {
      ...
      logger.debug('Active Call Page: Remote stream attached to video');
    }
  }, [remoteStream]);
  ...
  useEffect(() => {
    if (isConnected && !callStartTimeRef.current) {
      ...
      logger.debug('Active Call Page: ⏱️ Starting call timer');
  ...
  const handleEndCall = useCallback(async () => {
    ...
    try {
      logger.debug('Active Call Page: 📞 Ending call...');
      ...
      logger.error('Active Call Page: ❌ Failed to end call:', error);
      setIsEnding(false);
    }
  }, [activeCall, isEnding, cleanup, cleanupStreams, endCall, navigate]);
